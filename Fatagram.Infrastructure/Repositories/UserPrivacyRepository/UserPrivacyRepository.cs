...
namespace Fatagram.Infrastructure.Repositories.UserPrivacyRepository
{
    ...
    public class UserPrivacyRepository : IUserPrivacyRepository
    {
        ...
        public async Task<Dictionary<string, PrivacyLevel>> GetAsync(
            Guid userId,
        ...
        )
        {
            ...
            {
                var privacy = await _dbContext.UserPrivacies.FirstOrDefaultAsync(up =>
                    up.UserId == userId && up.Field == f
                );
                if (privacy != null)
                {
                    ...
                    userPrivacy.Add(f, PrivacyLevel.Public);
            }
            return userPrivacy;
        }
        ...
        public async Task InitialAsync(Guid userId)
        {
            var fields = await _dbContext
                .UserPrivacies.Where(up => up.UserId == userId)
                .ToListAsync();
        ...
        {
            var privacy = await _dbContext.UserPrivacies.FirstOrDefaultAsync(up =>
                up.UserId == userPrivacy.UserId && up.Field == userPrivacy.Field
            );
            if (privacy != null)
            {
